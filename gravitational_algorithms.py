...
SPEED_TOL = 0.05                # launch-speed resolution (m/s) for the bounded scan
MAX_SCAN_STEPS = 10000          # safety cap on scan iterations
...
def _draw_topology_axes(topology: TOPOLOGY, title: str, node_style: dict | None = None,
    ...
    plt.figure()
    node_style = node_style or {"marker": "o", "color": "gray", "s": 3}
    edge_style = edge_style or {"color": "lightgray", "linewidth": 1, "alpha": 0.3}
    node_x, node_y, edge_x, edge_y = _topology_arrays(topology)
    for i in range(0, len(edge_x), 2):
        plt.plot(edge_x[i:i + 2], edge_y[i:i + 2], **edge_style)
    plt.scatter(node_x, node_y, **node_style)
...
def _plot_trajectory(ax_or_none, topology: TOPOLOGY, trajectory: list, color: str,
    ...
    if not xs:
        return
    plt.scatter(xs[0], ys[0], marker="o", color=start_color)
    if mark_end:
        plt.scatter(xs[-1], ys[-1], marker="o", color=color)
    for i in range(0, len(xs), 1):
        plt.plot(xs[i:i + 2], ys[i:i + 2], color=color, linewidth=lw)
...
def trajectory_search(topology: TOPOLOGY, init_node_id: str, dest_node_id: str, init_angle: float,
    ...
    """Sweep the launch speed and record every distinct reachable trajectory.

    Returns ``{launch_speed: [node ids]}`` -- one entry per speed at which the
    trajectory changes.  A bounded exponential phase establishes the search
    range (capped at ``2**MAX_EXPONENT`` to avoid float64 overflow when the
    destination is the non-existent probe node), followed by a linear scan at
    ``SPEED_TOL`` resolution.
    """
    ...
    trajectory_dict = {lower_speed: lower_trajectory}
    scan_speed, steps = lower_speed, 0
    previous_trajectory = lower_trajectory
    while scan_speed < higher_speed and steps < MAX_SCAN_STEPS:
        scan_speed = min(scan_speed + SPEED_TOL, higher_speed)
        scan_xy = speed_xy_calculation(scan_speed, init_angle)
        scan_trajectory = _run(scan_xy[1])
        if _comparison_trajectory(scan_trajectory) != _comparison_trajectory(previous_trajectory):
            trajectory_dict[scan_speed] = scan_trajectory
            previous_trajectory = scan_trajectory
        steps += 1
...
def heat_map_plotting(topology: TOPOLOGY, grid_size: int, region_division: int,
    ...
    for i, color in enumerate(color_list):
        ...
        if i == len(color_list) - 1:
            color_code[(upper, 0)] = color
        else:
            color_code[(upper, lower)] = color
        upper = lower
...
def main():
    ...
    mean, stdev, cv = coefficient_of_variation_calculation(topology, args.grid_size)
    log.info("%d nodes | mean degree %.3f (sd %.3f, CV %.3f) | degree min %d / max %d | avg degree %.2f",
             args.nodes, mean, stdev, cv, min(degrees), max(degrees), topology.average_degree())
    ...
    if args.experiment in ("all", "reachability", "heatmap"):
        ...
        for start_node in sweep_nodes:
            ...
            extensive_non_reachable[start_node.id] = non_reachable
            total = len(all_nodes) - 1
            log.info("Node #%s: %d/%d reachable (%.1f%%) | non-reachable: %s",
                     start_node.id, len(reachable), total,
    ...
    if args.experiment in ("all", "trajectories", "reverse"):
        ...
        log.info("Trajectory search demo from node %s (angle %.3f rad, speed_x in -10..10)",
                 start_id, init_angle)
        demo_outputs = {}
        for speed_x in (-5, 0, 5):
            demo_outputs[speed_x] = trajectory_search(
    ...
    log.info("Execution time: %.3f s", time.process_time() - t_start)
